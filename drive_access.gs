...
var WHOHASACCESS_NOTIFICATION_RECIPIENTS = "your+whohasaccess@domain.edu";
// Google Sheet URL that you have access to edit (should be blank to begin with)
var WHOHASACCESS_GOOGLE_SHEET_URL = "https://docs.google.com/spreadsheets/d/1UR-1Z07GtDKWl423dsfdsfLvnkYMcJXI/edit";
// tab/sheet name to house the list of File IDs for everything in your Google Drive
var WHOHASACCESS_GOOGLE_SHEET_RESULTS_TAB_NAME = "newresults";
// change TIMEOUT VALUE
//var TIMEOUT_VALUE_MS = 270000; // 4.5 minutes
//var TIMEOUT_VALUE_MS = 210000; // 3.5 mins (so we can run this every 5 minutes)
var WHOHASACCESS_TIMEOUT_VALUE_MS = 210000; // 3.5 mins (so we can run this every 5 minutes)
...
function set_sheet_headers() {
  ...
  var results_sheet = SpreadsheetApp.openByUrl(WHOHASACCESS_GOOGLE_SHEET_URL).getSheetByName(WHOHASACCESS_GOOGLE_SHEET_RESULTS_TAB_NAME);
...
function lookup_all_google_drive_files_using_continuation_tokens() {
  ...
  var ss = SpreadsheetApp.openByUrl(WHOHASACCESS_GOOGLE_SHEET_URL).getSheetByName(WHOHASACCESS_GOOGLE_SHEET_RESULTS_TAB_NAME);
  ...
  if(!filesFromToken.hasNext()) {
    ...
    var templ = HtmlService
      .createTemplateFromFile('5-drive-access-email');
  
    templ.sheetUrl = WHOHASACCESS_GOOGLE_SHEET_URL;
    
    var message = templ.evaluate().getContent();
    
    MailApp.sendEmail({
      to: WHOHASACCESS_NOTIFICATION_RECIPIENTS,
...
function isTimeUp_(start) {
  var now = new Date();
  return now.getTime() - start.getTime() > WHOHASACCESS_TIMEOUT_VALUE_MS; // milliseconds
}
